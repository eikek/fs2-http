      ...
      _.pull.unconsChunk.flatMap {
        ...
        case Some((chunk, next)) =>
          ...
          if (all.size < 2) (next through dropInitial(all)).pull.echo
            ...
            if (all.startsWith(StartBom)) Pull.output(ByteVectorChunk(all.drop(2)).toSegment) >> next.pull.echo
            else Pull.output(ByteVectorChunk(all).toSegment) >> next.pull.echo
          }
      }.stream
    }
