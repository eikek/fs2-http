      ...
      in.pull.unconsChunk.flatMap {
        ...
        case Some((h, tl)) =>
          ...
          expect match {
            ...
            case Right(remains) =>
              if (remains == bv.size) Pull.output(ByteVectorChunk(bv).toSegment) >> go(Left(ByteVector.empty), tl)
              else if (remains > bv.size) Pull.output(ByteVectorChunk(bv).toSegment) >> go(Right(remains - bv.size), tl)
              else {
                val (out,next) = bv.splitAt(remains.toInt)
                Pull.output(ByteVectorChunk(out).toSegment) >> go(Left(ByteVector.empty), Stream.chunk(ByteVectorChunk(next)) ++ tl)
              }
          }
